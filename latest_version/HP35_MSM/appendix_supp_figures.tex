...
\begin{figure}[!t]
    ...
    Top row: values of the first non-trivial \textit{right}(*) eigenvector  projected onto the first two components of the reduced coordinate space. (*) Deeptime \cite{deeptime} represents transition matrices in the row-stochastic notation.
...
\begin{figure}[h!]
    ...
    \includegraphics[width=\linewidth]{figures/f1_f2_dwell_time_survival.pdf}
    \caption{\small\textbf{Analysis of subsets $F_1$ and $F_2$ identified by dihedral-based models.} \textbf{Top}: first $\simeq 40\,\mu s$ of the discretized trajectory ($F_1, F_2, \textrm{other}$) highlight $F_1\leftrightarrow F_2$ transitions. \textbf{Bottom}: survival probability (1-cdf) and linear fit shows the distribution of the residence time is well approximated by an exponential. }
