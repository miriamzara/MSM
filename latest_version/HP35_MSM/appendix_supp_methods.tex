...
\label{supsec:derivation_PCA_TICA}
...
With this normalization, the coordinates representing the slowest processes "weight more", and a unit step in those directions in the reduced space is assigned a larger distance.
...
\end{equation}
With this normalization, Euclidean distances approximate commute distances, i.e. the expected time required to travel from one configuration to another and back. This is a variation on the same idea behind the kinetic map, being eigenvalues and timescales connected by \eqref{eq:eigvals_and_timescales}. Both normalizations are defined in Deeptime \cite{deeptime}.
%
%
%
%
...
\label{supsec:markov_theory}
\paragraph{Transition matrix and spectral decomposition.}
A Markov state model approximates the conformational dynamics as a
memoryless jump process on the discrete microstates, encoded in the
transition matrix $T(\tau_{MSM})$ estimated at a fixed lag time $\tau_{MSM}$.
Following the convention used in Deeptime \cite{deeptime}, consider
$T$ row-stochastic,
\begin{equation}
  \sum_j T_{ij} = 1, \qquad T_{ij} \geq 0,
  \label{eq:rowstoch}
\end{equation}
so that $T_{ij}$ is the conditional probability of being in state $j$
one lag step after being in state $i$. Probability distributions are
represented as row vectors and evolve by right multiplication,
\begin{equation}
  p^{(n)} = p^{(0)}\,T^{\,n}.
  \label{eq:propagation}
\end{equation}
The stationary distribution $\pi$ is the left eigenvector with unit
eigenvalue, $\pi T = \pi$ with $\sum_i \pi_i = 1$, and the estimator used
here enforces detailed balance, $\pi_i T_{ij} = \pi_j T_{ji}$.

$T$ it admits the spectral decomposition
\begin{equation}
  T = \sum_{i=1}^{N} \lambda_i\, r_i\, l_i^{\mathsf T},
  \label{eq:spectral}
\end{equation}
where the eigenvalues are ordered as $1 = \lambda_1 > \lambda_2 \geq
\dots$, and $r_i$, $l_i$ are the right and left eigenvectors,
\begin{equation}
  T r_i = \lambda_i r_i, \qquad l_i^{\mathsf T} T = \lambda_i l_i^{\mathsf T},
  \label{eq:eigvecs}
\end{equation}
normalised to be bi-orthonormal, $l_i^{\mathsf T} r_j = \delta_{ij}$.
The leading pair is fixed by $r_1 = \mathbf{1}$ (the constant vector,
since $T$ is row-stochastic) and $l_1 = \pi$. Substituting
\eqref{eq:spectral} into \eqref{eq:propagation} and using
bi-orthonormality, any initial distribution relaxes to equilibrium as a
sum of exponentially decaying modes,
\begin{equation}
  p^{(n)} = \sum_{i} c_i\, \lambda_i^{\,n}\, l_i^{\mathsf T},
  \qquad c_i = p^{(0)} r_i .
  \label{eq:modeexpansion}
\end{equation}
The dominant term ($\lambda_1 = 1$) is the stationary distribution $\pi$;
because $|\lambda_i| < 1$ for $i \geq 2$, every other mode decays with a
characteristic relaxation (implied) timescale
\begin{equation}
  t_i = -\,\frac{1}{\ln \lambda_i}.
  \label{eq:timescale}
\end{equation}
where time is expressed in units the Markov step, or explicitly
\begin{equation}
  t_i = -\,\frac{\tau_{MSM}}{\ln \lambda_i}.
  \label{eq:timescale}
\end{equation}
in another unit of measure.
\paragraph{Relaxation modes and probability flux.}
The left eigenvectors $\{l_i\}_{i\geq 2}$ describe the spatial structure
of the relaxation processes: states whose components have opposite sign
exchange probability on the associated timescale $t_i$, and the
magnitude of each component measures how strongly that state
participates in the process. The redistribution driven by these modes
over a single lag step is obtained by expanding a distribution in the
same basis, $p = \sum_i c_i l_i^{\mathsf T}$ with $c_i = p\,r_i$, and
forming the difference
\begin{equation}
  \Delta p = pT - p
           = \sum_{i \geq 2} c_i\,(\lambda_i - 1)\, l_i^{\mathsf T},
  \label{eq:flux}
\end{equation}
where the stationary term drops out because $\lambda_1 = 1$. Equation
\eqref{eq:flux} makes explicit that only the sub-unit eigenvalues carry
probability flux, that the slowest modes ($\lambda_i \to 1$) produce the
smallest per-step change and therefore dominate the long-time
relaxation, and that the geometric pattern of each flux contribution is
set by the corresponding left eigenvector $l_i$.

For a reversible chain the left and right eigenvectors are not
independent but are related through the stationary distribution,
\begin{equation}
  l_i = \Pi\, r_i, \qquad \Pi = \operatorname{diag}(\pi),
  \label{eq:detbal}
\end{equation}
that is, $l_i(x) = \pi_x\, r_i(x)$. The right eigenvectors are thus the
equilibrium-population-weighted counterparts of the relaxation modes,
and they are orthonormal in the $\pi$-weighted inner product,
\begin{equation}
  \langle r_i, r_j \rangle_\pi
    := \sum_x \pi_x\, r_i(x)\, r_j(x) = \delta_{ij}.
  \label{eq:piorth}
\end{equation}
The sign structure of the dominant eigenvectors provides the geometric
criterion used by PCCA++~\cite[Ch.~2.5.1--2.5.2]{thebible} to lump
microstates into metastable macrostates. 

\begin{comment}
\paragraph{Implied-timescale test and lag-time selection.}
Markovianity was assessed, and the lag time selected, through the
implied-timescale (ITS) convergence test. The estimated timescales
\eqref{eq:timescale} are variational lower bounds on the true timescales
$t_i^\ast$ of the underlying continuous-state process,
$t_i(\tau) \leq t_i^\ast$, with the gap shrinking as the discretisation
is refined or as $\tau$ is increased. In practice $t_i(\tau)$ is plotted
against $\tau$ and one looks for a range over which the timescales are
approximately constant; the lag time is taken as the smallest $\tau$
(counted in frames, $\tau = 1, 2, 3, \dots$) inside this range, which
minimises the loss of temporal resolution while keeping the process of
interest approximately Markovian~\cite[Ch.~4.7]{thebible}. Because the
number of microstates and the lag time jointly control resolution and
statistical accuracy, the two were chosen together (see main text,
Sec.~III~A).
\end{comment}
\end{comment}